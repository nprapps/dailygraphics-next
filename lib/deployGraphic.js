...
module.exports = async function(config, slug) {
  ...
  for (var i = 0; i < matching.length; i++) {
    ...
    switch (extension) {
      ...
      case ".js":
        var result = await processJS(full, { production: true, root: config.root });
        contents = result.src;
        // upload the source map
        var mapPath = path.join(config.s3.prefix, slug, relative + ".map");
        await s3.upload(config.s3.bucket, mapPath, result.map);
        break;
