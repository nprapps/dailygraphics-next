...
var exporters = {
  s3: function(config) {
    return function(file, contents) {
      var staging = config.argv.target == "stage";
      var prefix = staging ? config.s3.stage.prefix : (config.s3.prefix || config.s3.live.prefix);
      var bucket = staging ? config.s3.stage.bucket : (config.s3.bucket || config.s3.live.bucket);
